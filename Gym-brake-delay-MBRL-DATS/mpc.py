...
class MPC:
    def __init__(self, env, plan_horizon, model, popsize, num_elites, max_iters,
        ...
        self.init_var = np.tile(np.ones(self.action_dim) * (0.5 ** 2),  # std = 0.5
                                self.plan_horizon)  # size: (action_dim * plan_horizon,)
        self.previous_actions = np.zeros(self.num_delay)

        # Controller initialization: dynamics model: predict transition s' = s + f(s, a; phi)
        self.train_in = np.array([]).reshape([0, self.state_dim + DELAY_STEPS * self.action_dim +self.action_dim])
    ...
    def get_action_cost(self, ac_seqs):
        ...
        previous_acs = np.tile(self.previous_actions[None], [popsize * self.num_particles, 1])
        ...
        for t in range(self.plan_horizon):
            ...
            next_obs = self.predict_next_state(cur_obs, previous_acs, cur_acs)
    ...
    def predict_next_state_model(self, states, previous_actions, actions):
        """ 
        ...
        states = torch.tensor(states) # here the states does not include the previous actions
        previous_actions = torch.tensor(previous_actions)
        actions = torch.tensor(actions).repeat_interleave(self.num_particles, dim=0)
        inputs = torch.cat((states, previous_actions, actions), axis=1).float()
        ...
        next_states = np.random.normal(ensemble_means, sd) + states[:,0:2]
        return next_states
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs, pre_acs_trajs, real_ob_trajs, num_train_itrs=5):
        """ 
        ...
        for obs, acs, pre_acs, real_obs in zip(obs_trajs, acs_trajs, pre_acs_trajs, real_ob_trajs):
            # input (state, action) 
            new_train_in.append(np.concatenate([obs[:-1, 0:2], pre_acs, acs], axis=-1))
            # predict dynamics: f(s, a, phi)    
            new_train_targs.append(real_obs[1:, 0:2] - real_obs[:-1, 0:2])
    ...
    def act(self, state, previous_actions, t):
        """
        ...
        self.previous_actions = np.array(previous_actions)
        self.goal = state[-1]
        ...
        return self.act(state, previous_actions, t)
